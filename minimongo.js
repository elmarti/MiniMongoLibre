 ...
 //     self.collection._observeQueue.drain();
  ...
  LocalCollection.prototype.insert = function(doc, callback) {
    ...
    if (!_.has(doc, '_id')) {
      ...
      doc._id = LocalCollection._useOID ? new MongoID() : Random.id();
    }
    var id = doc._id;
 ...
 //   self._observeQueue.drain();
  ...
  projectionDetails = function(fields) {
    ...
    if (fieldsKeys.length > 0 &&
      ...
      fieldsKeys = _.reject(fieldsKeys, function(key) {
        return key === '_id';
      });
  ...
  LocalCollection._IdMap = function() {
    var self = this;
    IdMap.call(self, /* MongoID.idStringify, MongoID.idParse*/ function(asd, QWE, ZXC) {
      console.log("CB1", asd, QWE, ZXC);
    }, function(asd, QWE, ZXC) {
      console.log("CB2", asd, QWE, ZXC);
    });
  };
  ...
  LocalCollection._CachingChangeObserver = function(options) {
    ...
    if (self.ordered) {
      self.docs = new OrderedDict(MongoID());
      self.applyChange = {
